...
class LiMBSParser:
    ...
    def _parse_resolution(self, part: str) -> str:
        m = re.fullmatch(r"rsln:(AA|CG|SCG)", part)
    ...
    def _parse_solvent(self, part: str) -> str:
        m = re.fullmatch(r"sol:([A-Za-z][A-Za-z0-9_+-]*)", part)
...
def canonicalize(system: LiMBSSystem) -> str:
    ...
    system_parts.extend(
        [
            ...
            (
                f"box:[{system.box['values'][0]:.1f},"
                f"{system.box['values'][1]:.1f},"
                f"{system.box['values'][2]:.1f}]"
                f"{system.box['unit']}"
            ),
